...
export async function downloadAndStoreTrack(orderId: string, externalUrl: string, index: number): Promise<string> {
...
export async function storeAllTracks(orderId: string, externalUrls: string[]): Promise<{ localUrls: string[]; errors: string[] }> {
  const localUrls: string[] = []
  const errors: string[] = []

  for (let i = 0; i < externalUrls.length; i++) {
    try {
      const localUrl = await downloadAndStoreTrack(orderId, externalUrls[i], i)
